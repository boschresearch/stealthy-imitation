...
from ding.worker import BaseLearner, create_serial_evaluator
from utils import (
    construct_transfer_dataset,
    load_victim_policy_eval,
    create_evaluator_and_logger
)

from utils_all import  calculate_budgets, p_test_analysis, plot_dists,  create_reward_model_and_collector
...
def initialize_estimate_distribution(cfg, args):
    ...
    real_mean = real_obs_data['mean'].cpu()
    real_std = real_obs_data['std'].cpu()
...
def create_check_list_evaluator(cfg, args,adversary_policy):
    env_fn, collector_env_cfg, evaluator_env_cfg = get_vec_env_setting(cfg.env)

    check_list_env = create_env_manager(cfg.env.manager, [partial(env_fn, cfg=c) for c in evaluator_env_cfg])
    check_list_env.seed(cfg.seed, dynamic_seed=False)
    tb_logger_check_list = SummaryWriter(f'./{cfg.exp_name}/check_list/log/serial')
    check_list_evaluator = create_serial_evaluator(
        cfg.policy.eval.evaluator,
        env=check_list_env,
        policy=adversary_policy.eval_mode,
        tb_logger=tb_logger_check_list,
        exp_name=f"{cfg.exp_name}/check_list"
    )
    monitor_dic = {
        'monitor_best_estimate_dist': [],
    ...
    return monitor_dic, check_list_evaluator
...
def main(args):
    ...
    victim_policy, victim_cfg = load_victim_policy_eval(args.victim_cfg, seed=args.seed)
    ...
    if args.check_list is not None:
        monitor_dic, check_list_evaluator = create_check_list_evaluator(cfg, args, adversary_policy)
    ...
    with torch.no_grad():
        stop, eval_info = evaluator.eval(learner.save_checkpoint, global_epoch, global_budget)  
    ...
    while(global_budget + query_budget + args.last_bc_budget <= monitor_dic['total_budget'][check_list_idx]):
        ...
        current_data,global_budget = construct_transfer_dataset(global_budget, victim_policy._model,query_budget, estimate_dist,args, train_val_ratio,cfg=cfg)
    ...
    if args.check_list is not None:
        ...
        for check_idx in range(len(monitor_dic['total_budget'])):
            ...
            bc_data, _ = construct_transfer_dataset(0, victim_policy._model, query_budget, estimate_dist, args, train_val_ratio, cfg=cfg)
            ...
            adversary_policy._model.eval()
            with torch.no_grad():
                stop, eval_info = check_list_evaluator.eval(None, num_iter, total_budget)



if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="This script runs the main program with specified arguments.")

    parser.add_argument("--debug", type=int, default=1, help="Debug mode. Default is 1.")
    parser.add_argument("--victim_cfg", type=str,default="exp/hopper/sac_seed0", help="Configuration for the victim. Default is 'exp/hopper/sac_seed0'.")
    parser.add_argument("--env", type=str, default="hopper", help="Environment to use. Default is 'hopper'.")
    parser.add_argument("--policy", type=str,default="gail_estimate", help="Policy to use. Default is 'gail_estimate'.")
    parser.add_argument("--seed", type=int,default=4, help="Random seed. Default is 4.")
    parser.add_argument("--exp_name", type=str,default="test", help="Name of the experiment. Default is 'test'.")

    parser.add_argument("--bc_budget", type=int,default=100000, help="Budget for behavioral cloning. Default is 100k.")
    parser.add_argument("--bc_batch_size", type=int, default=512, help="Batch size for behavioral cloning. Default is 512.")
    parser.add_argument("--bc_lr", type=float, default=0.001, help="Learning rate for behavioral cloning. Default is 0.001.")
    parser.add_argument("--bc_num_epochs", type=int, default=1, help="Number of epochs for behavioral cloning. Default is 1.")
